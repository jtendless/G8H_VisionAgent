...
//========== 표준 I/O 기기 공통 커맨드 =========================================================================
	...
	// Network제품 각 슬레이브 기기의 파라미터 설정 값을 반환하는 함수
	DWORD   __stdcall AxlM3GetStationParameterwNop(long lBoardNo, long lModuleNo, WORD wNo, BYTE bSize, BYTE bModuleType, BYTE *pbParam);
	// Network제품 각 슬레이브 기기의 파라미터 값을 설정하는 함수
	DWORD   __stdcall AxlM3SetStationParameterwNop(long lBoardNo, long lModuleNo, WORD wNo, BYTE bSize, BYTE bModuleType, BYTE *pbParam);
...
//========== 표준 I/O 프로파일 커맨드 =========================================================================
	...
	// M3 전용 원점 검색시 마지막 원점 검색 후 원점 센서를 찾는 기능 사용 유무 설정 및 설정 값 가져오기
	// uUse: 1, 기능 사용
	// uUse: 0, 기능 사용(디폴트 값)
	DWORD   __stdcall AxmHomeSetM3FinalHomeSearching(long lAxisNo, DWORD uUse);
	DWORD   __stdcall AxmHomeGetM3FinalHomeSearching(long lAxisNo, DWORD *uUse);

	// M3 전용 원점 검색시 마지막 원점 검색 후 원점 센서를 찾는 기능 설정 및 설정 값 가져오기
	// dLimitDistance:원점 검색 찾는 동작시 이동 제한 값 설정
	// dMoveDistance:원점 검색 찾는 동작시 한 번에 이동되는 거리 설정
	DWORD   __stdcall AxmHomeSetM3FinalHomeParameter(long lAxisNo, double dLimitDistance, double dMoveDistance);
	DWORD   __stdcall AxmHomeGetM3FinalHomeParameter(long lAxisNo, double *dLimitDistance, double *dMoveDistance);
