...
// Run function - override to implement a new behaviour
    virtual void Run();
	void OnBarcodeDataReceived(const LPBYTE lpBuffer, DWORD dwCount);
// Data function
...
private:
	/* Command Lists */
	CSr1000CmdStartReading	CmdStartReading;
	CSr1000CmdReset			CmdReset;

	int MakeCommMessage(LPSTR lpMessageBuffer, int nBufferLen, LPCTSTR lpszCmd);
	int StripeCommMessage(LPTSTR lpBuffer, int nBufferLen, LPCSTR lpMessage);
	void WaitResult(CSr1000CmdBase& rCmd, int nTimeOutMs = 3000);
private:
	CSr1000CmdBase*		pRequestedCmd;		/* Pointer to current pending command */
public:
	void ReadCode(bool bBlock = true, int nTimeOutMs = 3000);
	void Reset(bool bBlock = true, int nTimeOutMs = 3000);
	LPCTSTR GetLastCode(void);
	bool IsLastReadOk(void) { return CmdStartReading.IsLastReadOk(); }
