...
bool SockAddrIn::CreateFrom(LPCTSTR sAddr, LPCTSTR sService, int nFamily /*=AF_INET*/)
{
	...
	//sin_port = CSocketComm::GetPortNumber(sService);
	sin_family = nFamily;
...
void CSocketComm::OnBarcodeDataReceived(const LPBYTE lpBuffer, DWORD dwCount)
{
	TCHAR tBuffer[128];
	CHAR cMessage[128];

	int nLen = 0;
	unsigned int nGetBytes = 0;

	nGetBytes = dwCount;

	if (nGetBytes < dwCount)
	{
		/* truncated message */
	}

	memcpy(cMessage, lpBuffer, nGetBytes);
	cMessage[nGetBytes] = 0x00;

	nLen = StripeCommMessage(tBuffer, 128, cMessage);

	if (pRequestedCmd == NULL)
	{
		/* Discard Message */
		return;
	}

	pRequestedCmd->Parse(tBuffer, nLen);
}
...
bool CSocketComm::CreateSocketEx(LPCTSTR strHost, LPCTSTR strServiceName, int nFamily, int nType, UINT uOptions /* = 0 */)
{
    ...
    if (INVALID_SOCKET != sock)
    {
        ...
        SockAddrIn sockAddr;
#ifndef SIM_MODE
		strHost = _T("192.168.1.101");		
#else

		strHost = _T("192.168.3.93");
#endif
...
bool CSocketComm::ConnectTo(LPCTSTR strDestination, LPCTSTR strServiceName, int nFamily, int nType)
{
	...
	if (INVALID_SOCKET != sock)
	{
		...
		sockAddr.sin_addr.S_un.S_addr = inet_addr(CT2CA(strDestination));
  ...
  //      // try to connect - if fail, server not ready
		//if (SOCKET_ERROR == connect(sock, sockAddr.GetPort, sockAddr.Size()))
		//{
		//	int nErrorCode = WSAGetLastError();
        ...
        if (IsBroadcast() || bSmartAddressing )
        {
            ...
            if (bSmartAddressing)
            {
                ...
                if (sockAddr.sin_addr.s_addr != htonl(INADDR_BROADCAST))
                {
					...
					res = (int)sendto(s, lpszData, dwCount - nOffset, 0, sockAddr, (int)(sockAddr.Size()));
                ...
                {   // NOTE: broadcast will broadcast only to our peers
                    ...
                    for( ; iter != m_AddrList.end(); )
                    {
						...
						res = (int)sendto(s, (LPCSTR)&lpBuffer[nOffset], dwCount - nOffset, 0, sockAddr, (int)(iter->Size()));
                        if (res < 0)
                        {
...
void CSocketComm::Run()
{
    ...
    while( IsOpen() )
    {
        ...
        {
			if (bBarCodeFlag == false)
				OnDataReceived( lpData, dwBytes);
			else
				OnBarcodeDataReceived(lpData, dwBytes);
...
void CSocketComm::ReadCode(bool bBlock/* = true */, int nTimeOutMs/* = 3000 */)
{
	TCHAR tBuffer[128];
	CHAR cMessage[128];
	...
	CmdStartReading.Clear();
	CmdStartReading.MakeCommand(tBuffer, 128);

	nLen = MakeCommMessage(cMessage, 128, tBuffer);

	pRequestedCmd = dynamic_cast<CSr1000CmdBase*>(&CmdStartReading);

	WriteComm((LPBYTE)cMessage, nLen, INFINITE);
	...
	WaitResult(CmdStartReading, nTimeOutMs);

	bBarCodeFlag = false;
}

void CSocketComm::Reset(bool bBlock/* = true */, int nTimeOutMs/* = 3000 */)
{
	TCHAR tBuffer[128];
	CHAR cMessage[128];
	...
	CmdReset.Clear();
	CmdReset.MakeCommand(tBuffer, 128);

	nLen = MakeCommMessage(cMessage, 128, tBuffer);

	pRequestedCmd = dynamic_cast<CSr1000CmdBase*>(&CmdReset);
	...
	WaitResult(CmdReset, nTimeOutMs);
}

LPCTSTR CSocketComm::GetLastCode(void)
{
	if (CmdStartReading.IsLastReadOk() == false)
		return _T("");

	return CmdStartReading.GetLastCode();
}


void CSocketComm::WaitResult(CSr1000CmdBase& rCmd, int nTimeOutMs/* = 3000*/)
{
	int nSleepUnitMs = 100;
	int nElapsedMs = 0;

	for (; nElapsedMs < nTimeOutMs;)
	{
		if (rCmd.GetLastResult() != CSr1000CmdBase::eRESULT_NONE)
			break;

		Sleep(nSleepUnitMs);
		nElapsedMs += nSleepUnitMs;
	}

	if (nElapsedMs >= nTimeOutMs)
	{
		rCmd.SetLastResult(CSr1000CmdBase::eRESULT_FAIL);
	}
}


int CSocketComm::MakeCommMessage(LPSTR lpMessageBuffer, int nBufferLen, LPCTSTR lpszCmd)
{
	TCHAR tBuffer[128];
	int nLen = 0;
	...
	nLen = _stprintf_s(tBuffer, 128, _T("\x1B%s\x0D"), lpszCmd);

	if ((nLen <= 0) || (nLen > nBufferLen))
	{
		return -1;
	}

	CT2A ascII(tBuffer);
	strncpy_s(lpMessageBuffer, nBufferLen, ascII.m_psz, nLen);

	return nLen;
}

int CSocketComm::StripeCommMessage(LPTSTR lpBuffer, int nBufferLen, LPCSTR lpMessage)
{
	int nLen = 0;
	...
	strTemp = CA2T((LPSTR)lpMessage);

	strTemp.Replace(_T("\r"), _T("\0"));
	strTemp.Replace(_T("\n"), _T("\0"));

	nLen = _stprintf_s(lpBuffer, nBufferLen, _T("%s"), (LPCTSTR)strTemp);
