...
//IMPLEMENT_DYNAMIC(CRepeatListCtrl, CListCtrl)

CRepeatListCtrl::CRepeatListCtrl()
{
	HitItem = -1;
...
BEGIN_MESSAGE_MAP(CRepeatListCtrl, CListCtrlCustomDraw)
	//{{AFX_MSG_MAP(CRepeatListCtrl)
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
	...
	ON_WM_KILLFOCUS()
	ON_WM_DRAWITEM()
	ON_MESSAGE(WM_CHECKSTATUS, &CRepeatListCtrl::OnCheckStatus)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CRepeatListCtrl message handlers

BOOL CRepeatListCtrl::IsDraw()
{
	return true;
}

BOOL CRepeatListCtrl::OnDraw(CDC* pDC)
{
	/*
	DRAWITEMSTRUCT ds;
	CRect vrc;
...
BOOL CRepeatListCtrl::IsNotifyItemDraw()
{
	return true;
}

BOOL CRepeatListCtrl::IsNotifySubItemDraw(int /*nItem*/, UINT /*nState*/, LPARAM /*lParam*/)
{
	return true;
}

COLORREF CRepeatListCtrl::TextColorForSubItem(int nItem, int nSubItem, UINT /*nState*/, LPARAM /*lParam*/)
{
	if (0 == nSubItem) return RGB(0, 0, 0);
	else return CLR_DEFAULT;
}

COLORREF CRepeatListCtrl::BkColorForSubItem(int nItem, int nSubItem, UINT /*nState*/, LPARAM /*lParam*/)
{
	if (nItem % 2)
	{
...
///							For EditCell							///
///#################################################################///
void CRepeatListCtrl::Resize(int cx, int cy)
{
...
int CRepeatListCtrl::HitTestEx(CPoint& Point, int* pColumn)
{
	int ColumnNum = 0;
...
CEdit* CRepeatListCtrl::EditSubItem(int Item, int Column)
{
...
void CRepeatListCtrl::OnEndLabelEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
...
///							For ComboBox							///
///#################################################################///
CComboBox* CRepeatListCtrl::ShowList(int nItem, int nCol, CString StrData)
{
	CRect rect;
	int offset = 0;
...
void CRepeatListCtrl::SetListCtrlMode(int nInspectListMode)
{
	m_nInspectListMode = nInspectListMode;
}

void CRepeatListCtrl::InsertNumber(int Item, int Column)
{
...
int CRepeatListCtrl::InsertItemEx(int Item)
{
	CString ItemNum;
	ItemNum.Format(_T("%d"), Item + 1);
...
void CRepeatListCtrl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (GetFocus() != this)
		SetFocus();
...
void CRepeatListCtrl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (GetFocus() != this)
		SetFocus();
...
void CRepeatListCtrl::OnLButtonDown(UINT nFlags, CPoint Point)
{
...
void CRepeatListCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
void CRepeatListCtrl::InitialVelocityListCtrl()
{
	m_nInsertItemCount = 0;
	DeleteAllItems();
}

void CRepeatListCtrl::AddAllListData()
{
	/*
...
void CRepeatListCtrl::AddListColumn()
{
	LV_COLUMN lvcolumn;
	TCHAR *listcolumn[MAX_REPEAT_LIST_COL] = { _T("No") ,_T("X(mm)"), _T("Y(mm)"), _T("Z(mm)"), _T("Mark"), _T("Threshold"), _T("SizeX(mm)"), _T("SizeY(mm)"), _T("Find"), _T("Focus"), _T("Count"), _T("Pre_Co"), _T("Pre_R"), _T("Zoom_Co"), _T("Zoom_R"), _T("Zoom_B"), _T("FindType") };
	int width[MAX_REPEAT_LIST_COL] =		  { 40       , 100       , 100        , 100        , 80       , 80            , 100            , 100            , 80       ,80         , 80		, 80		  , 80		   , 80          , 80         , 80         , 80 };
...
BOOL CRepeatListCtrl::InitCheckCtrl()
{
	m_nSelectedItemNumber = -1;
	m_nUpdatePointNumber = -1;
...
BOOL CRepeatListCtrl::AddCheckCtrl(int nCol, int nCheckStatus)
{
	CRect rect;
	int offset = 0;
	int nItem = 0;// , nCol = 1;
...
BOOL CRepeatListCtrl::AddListDataString(int nRow, CString *strData)
{
	m_nSelectedItemNumber = -1;
	...
	for (int j = 0; j < MAX_REPEAT_LIST_COL; j++)
	{
		//sunghaklee
		//lvitem.iItem = (j == 0) ? MAX_REPEAT_LIST_COL : curitem;
		lvitem.iItem = (j == 0) ? m_nInsertItemCount : curitem;
		lvitem.iSubItem = j;
...
BOOL CRepeatListCtrl::AddListData(int nNo, double dValue[MAX_REPEAT_LIST_COL])
{
	m_nSelectedItemNumber = -1;
	m_nUpdatePointNumber = -1;
	...
	TCHAR  chData[MAX_REPEAT_LIST_COL][256];
...
BOOL CRepeatListCtrl::DeleteAllListData()
{
...
BOOL CRepeatListCtrl::DeleteListData(int nNo)
{
	int i;
	int nCount;
	TCHAR chData[256];
...
BOOL CRepeatListCtrl::InsertListData(int nNo)
{
	int i;
	int nCount;
	TCHAR chData[256];
...
int CRepeatListCtrl::SaveListData(BOOL bIsAuto /*= FALSE*/)
{
	return 1;
}


void CRepeatListCtrl::ColDataWrite(CStdioFile *file)
{
	CString szMsg = _T("");
...
CString CRepeatListCtrl::LoadDataFromCsv(CString strFilePath, CString *strResult, int *nRowQuan)
{
	CStdioFile file;
	CFileException ex;
	//char szContent[10000] = { 0, };
	double dValue[MAX_REPEAT_LIST_COL];
	int nRowCnt = 0;
	int nTrimPos = 0;
	CString szMsg;
	CString strData = _T("");
	...
	if (fOpenDlg.DoModal() == IDOK)
	{
		...
		if (file.Open(strPath, CFile::CFile::modeRead, &ex))
		{
			...
			while (file.ReadString(strContent)) //파일을 한 라인씩 읽어라
			{
				...
				{
					if(MAX_REPEAT_COUNT > nDataCount)
...
CString CRepeatListCtrl::SaveDataToCsv(CString strFilePath)
{
	SYSTEMTIME st;
	CStdioFile file;
	CFileException ex;
...
void CRepeatListCtrl::GetListData()
{
}

void CRepeatListCtrl::AddOffsetListData(int nIndex[2], double dPosData[MAX_REPEAT_LIST_COL])
{
...
BOOL CRepeatListCtrl::UpdateListData(double dValue[MAX_REPEAT_LIST_COL], int nPNTNumber)
{
	...
	CString StrUpdateData[MAX_REPEAT_LIST_COL];
...
int CRepeatListCtrl::GetMaxListCount()
{
	return m_nInsertItemCount;
}

void CRepeatListCtrl::ResetMaxListCount()
{
	m_nInsertItemCount = 0;
}

//int No, int nMaskY, int nCellXY[2], int nDivXY[2], double dPointXY[2]
BOOL CRepeatListCtrl::GetListData(int nRow, double dData[MAX_REPEAT_LIST_COL])
{
	//int i;
	CString Str;
...
BOOL CRepeatListCtrl::IsReal(CString Str)
{
	TCHAR ch;
...
void CRepeatListCtrl::HighlightSelectedItem()
{
...
int CRepeatListCtrl::GetItemHeight()
{
	CRect ItemRect;
...
void CRepeatListCtrl::SetScroll()
{
	SCROLLINFO sinfo;
...
void CRepeatListCtrl::OnKillFocus(CWnd* pNewWnd)
{
	CListCtrl::OnKillFocus(pNewWnd);
	HighlightSelectedItem();
}

void CRepeatListCtrl::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	// TODO: Add your message handler code here and/or call default
...
void CRepeatListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
BOOL CRepeatListCtrl::PreTranslateMessage(MSG* pMsg)
{
...
int CRepeatListCtrl::LoadPointData(char* pchPathName)
{
	/*
...
LRESULT CRepeatListCtrl::OnCheckStatus(WPARAM wParam, LPARAM lParam)
{
