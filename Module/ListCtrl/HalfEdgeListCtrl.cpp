...
//IMPLEMENT_DYNAMIC(CHalfEdgeListCtrl, CListCtrl)

CHalfEdgeListCtrl::CHalfEdgeListCtrl()
{
	HitItem = -1;
...
BEGIN_MESSAGE_MAP(CHalfEdgeListCtrl, CListCtrlCustomDraw)
	//{{AFX_MSG_MAP(CHalfEdgeListCtrl)
	ON_WM_HSCROLL()
	ON_WM_VSCROLL()
	...
	ON_WM_KILLFOCUS()
	ON_WM_DRAWITEM()
	ON_MESSAGE(WM_CHECKSTATUS, &CHalfEdgeListCtrl::OnCheckStatus)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CHalfEdgeListCtrl message handlers

BOOL CHalfEdgeListCtrl::IsDraw()
{
	return true;
}

BOOL CHalfEdgeListCtrl::OnDraw(CDC* pDC)
{
	/*
	DRAWITEMSTRUCT ds;
	CRect vrc;
...
BOOL CHalfEdgeListCtrl::IsNotifyItemDraw()
{
	return true;
}

BOOL CHalfEdgeListCtrl::IsNotifySubItemDraw(int /*nItem*/, UINT /*nState*/, LPARAM /*lParam*/)
{
	return true;
}

COLORREF CHalfEdgeListCtrl::TextColorForSubItem(int nItem, int nSubItem, UINT /*nState*/, LPARAM /*lParam*/)
{
	if (0 == nSubItem) return RGB(0, 0, 0);
	else return CLR_DEFAULT;
}

COLORREF CHalfEdgeListCtrl::BkColorForSubItem(int nItem, int nSubItem, UINT /*nState*/, LPARAM /*lParam*/)
{
	if (nItem % 2)
	{
...
///							For EditCell							///
///#################################################################///
void CHalfEdgeListCtrl::Resize(int cx, int cy)
{
...
int CHalfEdgeListCtrl::HitTestEx(CPoint& Point, int* pColumn)
{
	int ColumnNum = 0;
...
CEdit* CHalfEdgeListCtrl::EditSubItem(int Item, int Column)
{
...
void CHalfEdgeListCtrl::OnEndLabelEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
...
///							For ComboBox							///
///#################################################################///
CComboBox* CHalfEdgeListCtrl::ShowList(int nItem, int nCol, CString StrData)
{
	CRect rect;
	int offset = 0;
...
void CHalfEdgeListCtrl::SetListCtrlMode(int nInspectListMode)
{
	m_nInspectListMode = nInspectListMode;
}

void CHalfEdgeListCtrl::InsertNumber(int Item, int Column)
{
...
int CHalfEdgeListCtrl::InsertItemEx(int Item)
{
	CString ItemNum;
	ItemNum.Format(_T("%d"), Item + 1);
...
void CHalfEdgeListCtrl::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (GetFocus() != this)
		SetFocus();
...
void CHalfEdgeListCtrl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	if (GetFocus() != this)
		SetFocus();
...
void CHalfEdgeListCtrl::OnLButtonDown(UINT nFlags, CPoint Point)
{
...
void CHalfEdgeListCtrl::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
...
void CHalfEdgeListCtrl::InitialVelocityListCtrl()
{
	m_nInsertItemCount = 0;
	DeleteAllItems();
}

void CHalfEdgeListCtrl::AddAllListData()
{
	/*
...
void CHalfEdgeListCtrl::AddListColumn()
{
	LV_COLUMN lvcolumn;
...
BOOL CHalfEdgeListCtrl::InitCheckCtrl()
{
	m_nSelectedItemNumber = -1;
	m_nUpdatePointNumber = -1;
...
BOOL CHalfEdgeListCtrl::AddCheckCtrl(int nCol, int nCheckStatus)
{
	CRect rect;
	int offset = 0;
	int nItem = 0;// , nCol = 1;
...
BOOL CHalfEdgeListCtrl::AddListDataString(int nRow, CString *strData)
{
	m_nSelectedItemNumber = -1;
...
BOOL CHalfEdgeListCtrl::AddListData(int nNo, double dValue[MAX_HALF_EDGE_LIST_COL])
{
	m_nSelectedItemNumber = -1;
	m_nUpdatePointNumber = -1;
...
BOOL CHalfEdgeListCtrl::DeleteAllListData()
{
...
BOOL CHalfEdgeListCtrl::DeleteListData(int nNo)
{
	int i;
	int nCount;
	TCHAR chData[256];
...
BOOL CHalfEdgeListCtrl::InsertListData(int nNo)
{
	int i;
	int nCount;
	TCHAR chData[256];
...
int CHalfEdgeListCtrl::SaveListData(BOOL bIsAuto /*= FALSE*/)
{
	return 1;
}


void CHalfEdgeListCtrl::ColDataWrite(CStdioFile *file)
{
	CString szMsg = _T("");
...
CString CHalfEdgeListCtrl::LoadDataFromCsv(CString strFilePath, CString *strResult, int *nRowQuan)
{
	CStdioFile file;
	CFileException ex;
...
CString CHalfEdgeListCtrl::SaveDataToCsv(CString strFilePath)
{
	SYSTEMTIME st;
	CStdioFile file;
	CFileException ex;
...
void CHalfEdgeListCtrl::GetListData()
{
}

void CHalfEdgeListCtrl::AddOffsetListData(int nIndex[2], double dPosData[MAX_HALF_EDGE_LIST_COL])
{
...
BOOL CHalfEdgeListCtrl::UpdateListData(double dValue[MAX_HALF_EDGE_LIST_COL], int nPNTNumber)
{
...
int CHalfEdgeListCtrl::GetMaxListCount()
{
	return m_nInsertItemCount;
}

void CHalfEdgeListCtrl::ResetMaxListCount()
{
	m_nInsertItemCount = 0;
}

//int No, int nMaskY, int nCellXY[2], int nDivXY[2], double dPointXY[2]
BOOL CHalfEdgeListCtrl::GetListData(int nRow, double dData[MAX_HALF_EDGE_LIST_COL])
{
	//int i;
	CString Str;
...
BOOL CHalfEdgeListCtrl::IsReal(CString Str)
{
	TCHAR ch;
...
void CHalfEdgeListCtrl::HighlightSelectedItem()
{
...
int CHalfEdgeListCtrl::GetItemHeight()
{
	CRect ItemRect;
...
void CHalfEdgeListCtrl::SetScroll()
{
	SCROLLINFO sinfo;
...
void CHalfEdgeListCtrl::OnKillFocus(CWnd* pNewWnd)
{
	CListCtrl::OnKillFocus(pNewWnd);
	HighlightSelectedItem();
}

void CHalfEdgeListCtrl::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	// TODO: Add your message handler code here and/or call default
...
void CHalfEdgeListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
...
BOOL CHalfEdgeListCtrl::PreTranslateMessage(MSG* pMsg)
{
...
int CHalfEdgeListCtrl::LoadPointData(char* pchPathName)
{
	/*
...
LRESULT CHalfEdgeListCtrl::OnCheckStatus(WPARAM wParam, LPARAM lParam)
{
