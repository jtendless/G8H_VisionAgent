...
#define MAX_REPEAT_LIST_COL 17
class CRepeatListCtrl : public CListCtrlCustomDraw
{
	//DECLARE_DYNAMIC(CRepeatListCtrl)

public:
	CRepeatListCtrl();
	virtual ~CRepeatListCtrl();
...
public:
	...
	BOOL AddListData(int nNo, double dValue[MAX_REPEAT_LIST_COL]);
	...
	BOOL UpdateListData(double dPosData[MAX_REPEAT_LIST_COL], int SelectIndex);
	...
	void AddOffsetListData(int nIndex[2], double dPosData[MAX_REPEAT_LIST_COL]);

	int GetMaxListCount();
	void ResetMaxListCount();
	BOOL GetListData(int nRow, double dData[MAX_REPEAT_LIST_COL]);
