...
void ClrLampButton::SetColour(COLORREF new_text_colour, COLORREF new_background_colour)
{
...
void ClrLampButton::SetColour(COLORREF new_text_colour, COLORREF new_background_colour, COLORREF new_disabled_background_colour)
{
	text_colour					= new_text_colour;
	background_colour			= new_background_colour; 
...
void ClrLampButton::ResetColour()
{
	text_colour					= GetSysColor(COLOR_BTNTEXT);
	background_colour			= GetSysColor(COLOR_BTNFACE); 
	disabled_background_colour	= background_colour;
...
BEGIN_MESSAGE_MAP(ClrLampButton, CButton)
	//{{AFX_MSG_MAP(ClrLampButton)
		// NOTE - the ClassWizard will add and remove mapping macros here.
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// ClrLampButton message handlers

void ClrLampButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	CDC		*dc;
	...
	if (state & ODS_DISABLED)
	{
		...
		DrawButtonText(dc, text_rect, (LPSTR)buffer, CLR_BTN_DGREY);
    }
	else
	{
		if (state & ODS_SELECTED)
...
void CClrLampButton::DrawFrame(CDC *dc, CRect r, int state)
{ 
	COLORREF colour;
...
void CClrLampButton::DrawFilledRect(CDC *dc, CRect r, COLORREF colour)
{ 
	CBrush B;
...
void CClrLampButton::DrawLine(CDC *dc, long sx, long sy, long ex, long ey, COLORREF colour)
{ 
	CPen new_pen;
	CPen *old_pen;
...
void CClrLampButton::DrawButtonText(CDC *dc, CRect r, const char *Buf, COLORREF text_colour)
{
	...
	//dc->DrawText((LPSTR)Buf, strlen(Buf), r, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
	dc->DrawText(Str, nLen, r, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
