...
#if !defined(_XMLFILE_H_INCLUDED_)
#define _XMLFILE_H_INCLUDED_


//
#if ! defined(XMLFILE_WINDOWS)
#if defined(_WIN32) || defined(WIN32)
#define XMLFILE_WINDOWS
#endif // WIN32 or _WIN32
#endif // not XMLFILE_WINDOWS
#if _MSC_VER > 1000 // VC++
#pragma once
#if ! defined(XMLFILE_SAFESTR) // not VC++ safe strings
#pragma warning(disable:4996) // VC++ 2005 deprecated function warnings
#endif // not VC++ safe strings
#if defined(XMLFILE_STL) && _MSC_VER < 1400 // STL pre VC++ 2005
#pragma warning(disable:4786) // std::string long names
#endif // VC++ 2005 STL
#else // not VC++
#if ! defined(XMLFILE_STL)
#define XMLFILE_STL
#endif // not STL
#if defined(__GNUC__) && ! defined(XMLFILE_ICONV) && ! defined(XMLFILE_STDCONV) && ! defined(XMLFILE_WINCONV)
#if ! defined(XMLFILE_WINDOWS)
#define XMLFILE_ICONV
#endif // not Windows
#endif // GNUC and not ICONV not STDCONV not WINCONV
#endif // not VC++
#if (defined(_UNICODE) || defined(UNICODE)) && ! defined(XMLFILE_WCHAR)
#define XMLFILE_WCHAR
#endif // _UNICODE or UNICODE
#if (defined(_MBCS) || defined(MBCS)) && ! defined(XMLFILE_MBCS)
#define XMLFILE_MBCS
#endif // _MBCS and not MBCS
#if ! defined(XMLFILE_SIZEOFWCHAR)
#if __SIZEOF_WCHAR_T__ == 4 || __WCHAR_MAX__ > 0x10000
#define XMLFILE_SIZEOFWCHAR 4
#else // sizeof(wchar_t) != 4
#define XMLFILE_SIZEOFWCHAR 2
#endif // sizeof(wchar_t) != 4
#endif // not XMLFILE_SIZEOFWCHAR
#if ! defined(XMLFILE_WINCONV) && ! defined(XMLFILE_STDCONV) && ! defined(XMLFILE_ICONV)
#define XMLFILE_WINCONV
#endif // not WINCONV not STDCONV not ICONV
#if ! defined(XMLFILE_FILEBLOCKSIZE)
#define XMLFILE_FILEBLOCKSIZE 16384
#endif
...
#if defined(XMLFILE_SAFESTR) // VC++ safe strings
...
#if defined(__GNUC__) && ! defined(XMLFILE_WINDOWS) // non-Windows GNUC
#define MCD_SSZ(sz) sz,(sizeof(sz)/sizeof(MCD_CHAR))
...
// Define XMLFILE_STL to use STL strings
//
#if defined(XMLFILE_STL) // STL
#include <string>
#if defined(XMLFILE_WCHAR)
...
// afx.h provides CString, to avoid "WINVER not defined" #include stdafh.x in XMLFILE.cpp
#include <afx.h>
...
#if defined(XMLFILE_WCHAR) && defined(__GNUC__)
#undef MCD_FOPEN
#define MCD_FOPEN(f,n,m) f=fopen(n,m)
...
#elif _MSC_VER >= 1000 && defined(XMLFILE_HUGEFILE) // VC++ HUGEFILE
...
#if defined(XMLFILE_EOL_NONE)
#define MCD_EOL MCD_T("")
#elif defined(XMLFILE_EOL_RETURN) // rare; only used on some old operating systems
#define MCD_EOL MCD_T("\r")
#elif defined(XMLFILE_EOL_NEWLINE) // Unix standard
#define MCD_EOL MCD_T("\n")
#elif defined(XMLFILE_EOL_CRLF) || defined(XMLFILE_WINDOWS) // Windows standard
#define MCD_EOL MCD_T("\r\n")
...
{
public:
	CXmlFile() { x_InitXmlFile(); SetDoc(NULL); };
	CXmlFile(MCD_CSTR szDoc) { x_InitXmlFile(); SetDoc(szDoc); };
	CXmlFile(int nFlags) { x_InitXmlFile(); SetDoc(NULL); m_nDocFlags = nFlags; };
	CXmlFile(const CXmlFile& xmlFile) { x_InitXmlFile(); *this = xmlFile; };
	void operator=(const CXmlFile& xmlFile);
	~CXmlFile();
...
public:
	...
	void			etDocFlags(int nFlags) { m_nDocFlags = (nFlags & ~(MDF_READFILE | MDF_WRITEFILE | MDF_APPENDFILE)); };
	enum XmlFileDocFlags
	{
		MDF_UTF16LEFILE = 1,
		MDF_UTF8PREAMBLE = 4,
		MDF_IGNORECASE = 8,
	...
	// double
	bool			AddElem(MCD_CSTR szName, double dValue, int nFlags = 0) { return x_AddElem(szName, dValue, nFlags); };
	bool			InsertElem(MCD_CSTR szName, double dValue, int nFlags = 0) { return x_AddElem(szName, dValue, nFlags | MNF_INSERT); };
	bool			AddChildElem(MCD_CSTR szName, double dValue, int nFlags = 0) { return x_AddElem(szName, dValue, nFlags | MNF_CHILD); };
	bool			InsertChildElem(MCD_CSTR szName, double dValue, int nFlags = 0) { return x_AddElem(szName, dValue, nFlags | MNF_INSERT | MNF_CHILD); };
...
#define XMLFILE_SETDEBUGSTATE x_SetDebugState()
#else // not DEBUG
#define XMLFILE_SETDEBUGSTATE
#endif // not DEBUG
	...
	bool           x_AddElem(MCD_PCSZ pName, double dValue, int nFlags);
...
#endif // !defined(_XMLFILE_H_INCLUDED_)
